...
import com.ruin.masscan.IpPortExecuteCmd;


public class AssetIpPortMapCollector implements Callable<Map<String, String>>{
	...
	private static Map<String, String> ipports =new HashMap<>();
	...
	public AssetIpPortMapCollector(Semaphore semaphore, String IP, String ports ,String filepath,String pak) {
		this.semaphore = semaphore;
		this.IP = IP;
		this.ports = ports;
		this.filepath = filepath;
	...
	public Map<String, String> call() throws Exception {
		...
		Map<String, String> result = IpPortExecuteCmd.execCmd("./masscan "+IP+" -p"+ports+" --rate "+pak, filepath);
		if (ipports.get(result.get(IP))!=null) {
			ipports.put(IP, ipports.get(IP)+","+result.get(IP));
		}else {
			ipports.put(IP, result.get(IP));
		}
...
//		if (args.length>0) {
//			if (args[0].indexOf("-")>=0) {
//				AssetIpPortCollector.MasscanIpSegment(args);
//			}else if(args[0].indexOf("\\")>=0||args[0].indexOf("/")>=0){
//				AssetIpPortCollector.MasscanByFile(args);
//			}else {
//				AssetIpPortCollector.Masscan(args);
//			}
//			
//		}else {
...
//		}
		...
		//参数是否为空
		if (args.length>0) {
			Matcher matIp = r.matcher(args[0]);
			...
			//ip参数判断为ip段
			if (args[0].indexOf("-")>=0&&boolean1==true) {
...
// 		Map<String, String> ipports = new HashMap<>();
 		...
 		for(String port:ports){
 			for(String ip : list) {
 	 			future = threadPool.submit(new AssetIpPortMapCollector(semaphore, ip, port,filepath,pak));
 	 		}
 		}
		...
		while (true) {
			//判断线程是否结束
 			if(future.isDone()&&threadPool.isTerminated()){
				...
				String ipportsString = ipports.toString().replaceAll(",", ":");
...
//			//分割数据
//			String [] ip_prot = ipport.split("_");
////			if (countMap.get(ip_prot[0])<500) {
//				//数据是否为空
//				if (map.get(ip_prot[0]) != null) {
//					//数量是否小于500
//					if (countMap.get(ip_prot[0])<500) {
//						map.put(ip_prot[0], map.get(ip_prot[0])+","+ip_prot[1]);
//						countMap.put(ip_prot[0], countMap.get(ip_prot[0])+1);
//					}else {
//						map.put(ip_prot[0],"0-65535");
//					}
//				}else {
//					map.put(ip_prot[0], ip_prot[1]);
//					countMap.put(ip_prot[0],1);
//				}
//				
////			}
//			
...
//			
//			
////			NmapthreadPool.execute(new NmapIpPortCollector(semaphoreNmap,ip_prot[0],ip_prot[1],writePath));
//		}
		for(Map.Entry<String, String> entry : ipports.entrySet()){
			NmapthreadPool.execute(new NmapIpPortCollector(semaphoreNmap,entry.getKey(),entry.getValue(),writePath));
		}
