...
public class AssetCollectorBynewFixedThreadPool implements Callable<Set<String>>{
	...
	public AssetCollectorBynewFixedThreadPool(List<String> list, String ports) {

		

	}
	
	public AssetCollectorBynewFixedThreadPool(Semaphore semaphore, String IP, String ports) {
		this.semaphore = semaphore;
		this.IP = IP;
		this.ports = ports;
	}
	public AssetCollectorBynewFixedThreadPool(Semaphore semaphore, ArrayList<String> IPS, String ports, Set<String> ipports) {
		this.semaphore = semaphore;
		this.IPS = IPS;
		this.ports = ports;
		this.ipports = ipports;
	}
	public AssetCollectorBynewFixedThreadPool(String IP, String ports, Set<String> ipports) {
		this.IP = IP;
		this.ports = ports;
		this.ipports = ipports;
	}
	...
	public Set<String> call() throws Exception {
		...
		System.out.println("D:/Computer_Science/IP探测/masscan/masscan "+IP+" -p"+ports);
		Map<String, Set<String>> result = ExecuteCmd.execCmd("D:/Computer_Science/IP探测/masscan/masscan "+IP+" -p"+ports+" --rate 200", "");
	...
	public static void main(String [] args) {
		...
		for (String ip:list) {
			...
			Future<Set<String>> future = fixedThreadPool.submit(new AssetCollectorBynewFixedThreadPool(ip, ports1,ipports));
			try {
				ipports.addAll(future.get());
